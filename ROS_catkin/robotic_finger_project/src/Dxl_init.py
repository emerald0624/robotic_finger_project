...
DXL_POS_I_GAIN			= 0
DXL_POS_D_GAIN			= 0
DXL_POS_FEEDFORWARD_FIRST_GAIN  = 0 
DXL_POS_FEEDFORWARD_SECOND_GAIN = 0
...
def dxl_init():
    ...
    #DXL_P_GAIN_VALUE
    dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(portHandler, DXL_ID, ADDR_PRO_POS_P_GAIN, DXL_POS_P_GAIN)
    if dxl_comm_result != COMM_SUCCESS:
        print "%s" % packetHandler.getTxRxResult(dxl_comm_result)
    elif dxl_error != 0:
        print "%s" % packetHandler.getRxPacketError(dxl_error)
    else:
        print "POS p gain of Dynamixel has been successfully changed : %04d" %(DXL_POS_P_GAIN)
  
    #DXL_I_GAIN_VALUE
    dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(portHandler, DXL_ID, ADDR_PRO_POS_I_GAIN, DXL_POS_I_GAIN)
    if dxl_comm_result != COMM_SUCCESS:
        print "%s" % packetHandler.getTxRxResult(dxl_comm_result)
    elif dxl_error != 0:
        print "%s" % packetHandler.getRxPacketError(dxl_error)
    else:
        print "POS i gain of Dynamixel has been successfully changed : %04d" %(DXL_POS_I_GAIN)

    #DXL_D_GAIN_VALUE
    dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(portHandler, DXL_ID, ADDR_PRO_POS_D_GAIN, DXL_POS_D_GAIN)
    if dxl_comm_result != COMM_SUCCESS:
        print "%s" % packetHandler.getTxRxResult(dxl_comm_result)
    elif dxl_error != 0:
        print "%s" % packetHandler.getRxPacketError(dxl_error)
    else:
        print "POS d gain of Dynamixel has been successfully changed : %04d" %(DXL_POS_D_GAIN)

    #DXL_FEEDFORWARD_1st_VALUE
    dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(portHandler, DXL_ID, ADDR_PRO_FEEDFORWARD_FIRST_GAIN, DXL_POS_FEEDFORWARD_FIRST_GAIN)
    if dxl_comm_result != COMM_SUCCESS:
        print "%s" % packetHandler.getTxRxResult(dxl_comm_result)
    elif dxl_error != 0:
        print "%s" % packetHandler.getRxPacketError(dxl_error)
    else:
        print "feedforward 1st gain of Dynamixel has been successfully changed : %04d" %(DXL_POS_FEEDFORWARD_FIRST_GAIN)

    #DXL_FEEDFORWARD_2nd_VALUE
    dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(portHandler, DXL_ID, ADDR_PRO_FEEDFORWARD_SECOND_GAIN, DXL_POS_FEEDFORWARD_SECOND_GAIN)
    if dxl_comm_result != COMM_SUCCESS:
        print "%s" % packetHandler.getTxRxResult(dxl_comm_result)
    elif dxl_error != 0:
        print "%s" % packetHandler.getRxPacketError(dxl_error)
    else:
        print "feedforward 2st gain of Dynamixel has been successfully changed : %04d" %(DXL_POS_FEEDFORWARD_SECOND_GAIN)
